...
VFIO_PATH = '/etc/dracut.conf.d/10-vfio.conf'

VFIO_CONTENT = '''# Automatically generated by EnvyControl

force_drivers+=" vfio_pci vfio vfio_iommu_type1 vfio_virqfd "
'''

VFIO_PCI_PATH = '/etc/modprobe.d/vfio.conf'

VFIO_PCI_CONTENT = '''# Automatically generated by EnvyControl

options vfio-pci ids=10de:1f95,10de:10fa # SUPPLY YOUR OWN IDS HERE
'''
...
def _switcher(mode, display_manager = ''):
    ...
    if mode == 'integrated':
        ...
        try:
            # Blacklist all nouveau and Nvidia modules
            _create_file(BLACKLIST_PATH, BLACKLIST_CONTENT)
            # Load VFIO modules at boot time            
            _create_file(VFIO_PATH, VFIO_CONTENT)
            # Bind VFIO drivers to dGPU       
            _create_file(VFIO_PCI_PATH, VFIO_PCI_CONTENT)
        except Exception as e:
            print(f'Error: {e}')
            sys.exit(1)
        _rebuild_initramfs()
        ...
        if choice in yes:
            ...
            _create_file(MODESET_PATH, MODESET_PM) # pls no modeset 1
        else:
        ...
        print('Supported graphics modes: integrated, vfio, nvidia, hybrid')
        sys.exit(1)
    print(f'Graphics mode set to: {mode}\nPlease reboot your computer for changes to apply!')

def _cleanup():
    # Remove all files created by EnvyControl
    to_remove = (BLACKLIST_PATH,UDEV_INTEGRATED_PATH, VFIO_PATH, VFIO_PCI_PATH, UDEV_PM_PATH, XORG_PATH, EXTRA_PATH, '/etc/X11/xorg.conf.d/90-nvidia.conf', MODESET_PATH, LIGHTDM_SCRIPT_PATH, LIGHTDM_CONFIG_PATH)
...
def _query_mode():
    if os.path.exists(BLACKLIST_PATH) and os.path.exists(UDEV_INTEGRATED_PATH):
        mode = 'integrated'
    elif os.path.exists(VFIO_PATH) and os.path.exists(VFIO_PCI_PATH):
        mode = 'vfio'
...
def main():
    ...
    parser.add_argument('-s', '--switch', type=str, metavar='MODE', action='store', help='switch the graphics mode, supported modes: integrated, vfio, hybrid, nvidia')
